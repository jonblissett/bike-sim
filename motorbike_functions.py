...
from C_motorbike import chain_eff_single, motorbike_mech_base, motorbike_mech4, motorbike_mech2
import time
...
def lap_analyse3(TT_Sim, Ref_Race, v, first_corner, last_corner, filename_ref_map, filename_ref_brake,
                 structure_map, var_name_brake, rated_energy, initial_energy, n_series, enable_warnings, verbosity):
    ramp_start = 0.3
    ...
    TT_Sim['Vdc_sim'][first_corner] = battery_simple(0, rated_energy, initial_energy, n_series, verbosity)[0]
    TT_Sim['Vdc'] = np.array([])
    TT_Sim['P'] = {}
    ...
    for c in range(first_corner, min(locsmin.size - 1, last_corner)):
        TT_Sim = corner_sim_single(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp,
                   Course_map, Ref_Race, TT_Sim, enable_warnings, verbosity)

    if verbosity > 0:
        print('Final Energy=%dWh' % (TT_Sim['Energy']['Drive'] / 3600))
    if verbosity > 0:
        print('Final Voltage=%.1fV' % (TT_Sim['Vdc_sim'][c + 1]))
    ...
    if verbosity == 0:
        print('Results ,%dkW' % (TT_Sim['motor']['P_max']/1000),
              ',%dNm' % TT_Sim['motor']['T_max'],
              ',%dWh' % (TT_Sim['Energy']['Drive'] / 3600),
              ',%.2fmph' % (37.733 / TT_Sim['t'][-1] * 3600))
    return TT_Sim


def lap_analyse2_parallel(TT_Sim, Ref_Race, v, first_corner, last_corner, filename_ref_map, filename_ref_brake,
    ...
    ## MAYBE ALLOCATE EACH SEVERAL CORNERS TO EACH CORE~S@???????????????????
    ## so instead call the function this comment is in
    for c in range(first_corner, min(locsmin.size - 1, last_corner)):
        ar = view.apply_async(corner_sim, c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt,
                        ramp, Course_map, Ref_Race, TT_Sim, enable_warnings)
        async_results.append(ar)

    print(rc.wait(async_results))   # Wait until all tasks are done
    print(len(async_results))
    #for c in range(first_corner, min(locsmin.size - 1, last_corner)):
    #    corner_sim(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp,
    #               Course_map, Ref_Race, TT_Sim, enable_warnings)
    for c in range(first_corner, min(locsmin.size - 1, last_corner)):
        print(async_results[c].stdout)
    results = [ar.get() for ar in async_results]
    res_copy = results[0]

    Sim_res = {}
    #for k in results[0].keys():
    #    Sim_res[k] = tuple(Sim_res[k] for Sim_res in ds)
    for k in results[0].keys():
        Sim_res[k] = results[0][k]
    for c in range (1+first_corner, min(locsmin.size - 1, last_corner)):
        for k in results[c].keys():
            Sim_res[k] = np.append(Sim_res[k],results[c][k])

    # Bit of a kludge to convert dict type to mat_structure
    # THIS SHOULD BE ELIMINATED BY USING DICTS THROUGHOUT
    sio.savemat('temp', {'Sim_res': Sim_res}, oned_as='column')
    TT_Sim = sio.loadmat('temp', struct_as_record=False, squeeze_me=True)['Sim_res']
    remove('temp.mat')
...
def corner_sim(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp, Course_map, Ref_Race, TT_Sim, enable_warnings):
    ...
    wfwlimited = wfw[v_s > 0.9 * TT_Sim['Vdc_sim'][0]]
    print('WARNING VDC SET TO INITIAL VALUE', TT_Sim['Vdc_sim'][0], ' - NO FW CALC')
    ...
    T_motor.append(ramp_start * np.interp(V[-1] / (1.0 / 60 * TT_Sim['N'][1] / TT_Sim['N'][0] * 2 * np.pi * R[-1]),
    ...
    for time in t[1:]:
        if D[-1] < Ref_Race.Distance[corner_index[-1]]:
            ...
            T_motor.append(np.interp(V[-1] / (1.0 / 60 * TT_Sim['N'][1] / TT_Sim['N'][0] * 2 * np.pi * R[-1]),
                                     TT_Sim['motor']['w'] / np.pi * 30, TT_Sim['motor']['t'], 0.001, 0.001))
    ...
    # TT_Sim['P']['MotorLosses'] = np.hstack((TT_Sim['P']['MotorLosses'], torque / TT_Sim['N'][0] * TT_Sim['N'][1] / TT_Sim['constants']['Km']))
    ...
    p_drive_loss = 100 #  inverter_losses(TT_Sim['Vdc'], TT_Sim['Vs'], abs(TT_Sim['Iq']) / np.sqrt(2), TT_Sim['PF'], 82e-6, 13e3, 0.8,
                                  # 1, 0.95e-3, 0.54e-3, 12e-3, 25e-3, 9.5e-3)[0]
    print('VDC IS INCORRECT FOR INVERTER LOSSESS')
    print('INVERTER LOSSES NOT INCLUDED')
...
def corner_sim_single(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp,
                      Course_map, Ref_Race, TT_Sim, enable_warnings, verbosity):
    corner_index = np.arange(locsmin[c], locsmin[c + 1] - 1)
    # corner_index_end = locsmin[c + 1]
    dt_a = dt[0]
    ...
    [v_s, v_d, v_q, power_factor] = v_dq_pmsm(TT_Sim['motor']['Ke'], TT_Sim['motor']['poles'], TT_Sim['motor']['Rs'],
                                              TT_Sim['motor']['Ld'], TT_Sim['motor']['Lq'], 0, TT_Sim['Iq'],
                                              TT_Sim['Rpm'] / 30 * np.pi)
    TT_Sim['Vs'] = v_s  # np.hstack((TT_Sim['Vs'], v_s))
    TT_Sim['Vd'] = v_d  # np.hstack((TT_Sim['Vd'], v_d))
    TT_Sim['Vq'] = v_q  # np.hstack((TT_Sim['Vq'], v_q))
    ...
    p_drive_loss = inverter_losses(TT_Sim['Vdc'], TT_Sim['Vs'], abs(TT_Sim['Iq']) / np.sqrt(2),
                                   TT_Sim['PF'], 82e-6, 13e3, 0.8, 1, 0.95e-3, 0.54e-3, 12e-3, 25e-3, 9.5e-3)[0]
    # print('VDC IS INCORRECT FOR INVERTER LOSSESS')
    # print('INVERTER LOSSES NOT INCLUDED')
    # TT_Sim['P']['DriveLosses'] = np.hstack((TT_Sim['P']['DriveLosses'], p_drive_loss))
    TT_Sim['P']['DriveLosses'] = p_drive_loss
    ...
    TT_Sim['Energy']['Drive'] = np.trapz(TT_Sim['P']['Drive'], TT_Sim['t'])
    # print('WISH YOU WERE HERE')
    TT_Sim['Vdc_sim'][c + 1] = battery_simple(TT_Sim['Energy']['Drive'] / 3600, rated_energy, initial_energy,
                                              n_series, verbosity)[0]
    if verbosity > 1:
        print('Energy=%dWh' % (TT_Sim['Energy']['Drive'] / 3600))
        print('Voltage=%.1fV' % (TT_Sim['Vdc_sim'][c + 1]))
    return TT_Sim
...
def battery_simple(energy_used, rated_energy, initial_energy, n_series, verbosity):
    ...
    if v_now > 4.35 * n_series:
        print('Error: Battery probably on fire')
        status = 'fire'
    else:
...
def battery_heat(n_cells, IR_cell, m_cell, I_cell, t, verbosity):
    shc = 1014
