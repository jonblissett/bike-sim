...
def corner_sim_old(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp, Course_map, Ref_Race, TT_Sim, enable_warnings):
    corner_index = np.arange(locsmin[c], locsmin[c + 1] - 1)
    # corner_index_end = locsmin[c + 1]
    dt_a = dt[0]
    dt_b = dt[1]
    ...
    TT_Sim.Vdc = np.hstack((TT_Sim.Vdc, TT_Sim.Vdc_sim[c] * np.ones(TT_Sim.t.size - TT_Sim.Vdc.size)))
    print('whoohoo')
    p_drive_loss = 100 #  inverter_losses(TT_Sim.Vdc[0], TT_Sim.Vs, abs(TT_Sim.Iq) / np.sqrt(2), TT_Sim.PF, 82e-6, 13e3, 0.8,
                                  # 1, 0.95e-3, 0.54e-3, 12e-3, 25e-3, 9.5e-3)[0]
...
def corner_sim(c, locsmin, v_flag, accumulated_energy, rated_energy, initial_energy, n_series, v, dt, ramp, Course_map, Ref_Race, TT_Sim, enable_warnings):
    ...
    p_drive_loss = 100 #  inverter_losses(TT_Sim['Vdc'], TT_Sim['Vs'], abs(TT_Sim['P']['MotorLosses']) / np.sqrt(2), TT_Sim['PF'], 82e-6, 13e3, 0.8,
                                  # 1, 0.95e-3, 0.54e-3, 12e-3, 25e-3, 9.5e-3)[0]
    print('VDC IS INCORRECT FOR INVERTER LOSSESS')
    print('INVERTER LOSSES NOT INCLUDED')
    # TT_Sim['P']['DriveLosses'] = np.hstack((TT_Sim['P']['DriveLosses'], p_drive_loss))
