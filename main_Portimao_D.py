...
last_corner = 16#16     # 7#63  # set to large number to use all

end_dist = 4600  # Distance at lap end for timing

# Export parameters
filename_exp = 'Python_sim_Portimao_D.mat'
structure_exp = 'Portimao_sim_D'
# Import filenames
...
TT_Sim = {'N': ([83.0, 16.0]),
          'constants': {'Km': 257/977, 'cd': 0.43, 'area': 1, 'rho': 1.204, 'm': 235 + 90 - 50, 'p_tyre': 1.9,
                        'r': 2.18 / 2 / np.pi},
          'J': {'wheel': 1.1, 'motor': 0},
...
L_core = 150
TT_Sim['motor'] = {}
TT_Sim['motor']['T_max'] = 680 * TT_Sim['constants']['Km']
TT_Sim['motor']['W_max'] = 8700 / 30 * np.pi
...
ax1.plot([], [], color='b', label='Lean (Recorded)', linewidth=5)
ax1.plot([], [], color='g', label='Lean (Simulated)', linewidth=5)
sim_lean = np.interp(Ref_Race.Distance, TT_Sim.Distance, -TT_Sim.lean*180/np.pi)
ax1.stackplot(Ref_Race.Distance, Ref_Race.lean*180/np.pi, sim_lean-Ref_Race.lean*180/np.pi)
# Make the y-axis label and tick labels match the line color.
ax1.set_ylabel('Lean Angle (degrees)', fontsize=18)
ax1.set_ylim(-70, 110)
plt.xlim(250, 4500)
...
ax2 = ax1.twinx()
ax2.plot([],[],color='c', label='Speed (Recorded)', linewidth=5)
ax2.plot([],[],color='r', label='Speed (Simulated)', linewidth=5)
ax2.plot(Ref_Race.Distance, Ref_Race.vGPS, 'c-', TT_Sim.Distance, TT_Sim.v, 'r-')
...
# plt.show()
fig.savefig(structure_exp + '_Lean.png')
fig.show()
